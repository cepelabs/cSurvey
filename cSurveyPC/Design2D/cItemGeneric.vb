...
Namespace cSurvey.Design.Items
    Public Class cItemGeneric
        ...
        Friend Overrides Function ToSvgItem(ByVal SVG As XmlDocument, ByVal PaintOptions As cOptions, ByVal Options As cItem.SVGOptionsEnum) As XmlElement
            ...
            'Dim oSequences() As cSequence = MyBase.Points.GetSequences()
            ...
            '                    Call oBorderPath.AddCurve(oSequencePoints, 1)
            '                Case Else
            ...
            '    End Try
            'Next
            'Call modSVG.AppendItem(SVG, oSVGGroup, modSVG.CreateItem(PaintOptions, SVG, oBorderPath, oTranslation, MyBase.Brush, Nothing, MyBase.Name))
            ...
            '            Case cIItemLine.LineTypeEnum.Splines
            '                Call oPath.AddCurve(oSequencePoints, 1)
            '            Case Else
            '                Call oPath.AddLines(oSequencePoints)
            '        End Select
            '    Catch
            '    End Try
            '    Call modSVG.AppendItem(SVG, oSVGGroup, modSVG.CreateItem(PaintOptions, SVG, oPath, oTranslation, Nothing, oSequence.GetPen(MyBase.Pen), MyBase.Name))
        ...
        Friend Overrides Sub Render(ByVal Graphics As System.Drawing.Graphics, ByVal PaintOptions As cOptions, ByVal Options As cItem.PaintOptionsEnum, ByVal Selected As SelectionModeEnum)
            ...
            With oCache
                If .Invalidated Then
                    ...
                    '    Call modPaint.RenderPointToSegmentBindings(oCache, MyBase.Survey, Me)
                    'End If
        ...
        Private Sub pLoadData(ByVal Clipart As cDrawClipArt, ByVal Options As cItemGenericOptions)
            ...
            Call MyBase.Survey.RaiseOnProgressEvent("item.import", cSurvey.OnProgressEventArgs.ProgressActionEnum.Begin, "Importazione dati...", 0, cSurvey.OnProgressEventArgs.ProgressOptionsEnum.ImageLoad Or cSurvey.OnProgressEventArgs.ProgressOptionsEnum.ShowProgressWindow)
            For Each opath As cDrawPath In Clipart.Paths
                Call MyBase.Survey.RaiseOnProgressEvent("item.import", cSurvey.OnProgressEventArgs.ProgressActionEnum.Progress, "Importazione dati...(path " & iCurrent & " di " & iCount & " -> points:" & opath.Path.PointCount & ")", iCurrent / iCount)
                Try
                    Dim bFirst As Boolean = True
                    Dim oLastFirstPoint As PointF
                    If (opath.Path.PathPoints.LongLength < iSVGMaxPathLen) Or (iSVGMaxPathLen = 0) Then
                        For i As Long = 0 To opath.Path.PathPoints.LongLength - 1
                            ...
                            Dim oPoint As PointF = opath.Path.PathPoints(i)
                            Dim cPoint As cPoint = MyBase.Points.AddFromPaintPoint(oPoint)
                            If (iType And PathPointType.CloseSubpath) = PathPointType.CloseSubpath Then
                                Call MyBase.Points.AddFromPaintPoint(oLastFirstPoint)
                            Else
                                If iType = PathPointType.Start Or bFirst Then
                                    oLastFirstPoint = oPoint
                                    cPoint.BeginSequence = True
                                    bFirst = False
                                End If
                            End If
                        Next
                ...
                iCurrent += 1
            Next
            Call MyBase.Survey.RaiseOnProgressEvent("item.import", cSurvey.OnProgressEventArgs.ProgressActionEnum.End, "Importazione completata.", 0)
        ...
        Friend Overrides Function SaveTo(ByVal File As Storage.cFile, ByVal Document As XmlDocument, ByVal Parent As XmlElement, Options As cSurvey.SaveOptionsEnum) As XmlElement
            ...
            Call oItem.SetAttribute("linetype", iLineType)
            Return oItem
        End Function
