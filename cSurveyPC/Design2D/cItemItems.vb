...
Namespace cSurvey.Design.Items
    Public Class cItemItems
        ...
        Public Overrides Sub SetBindDesignType(ByVal BindDesignType As BindDesignTypeEnum, Optional ByVal BindSegment As Boolean = True)
            For Each oItem As cItem In oItems
                Call oItem.SetBindDesignType(BindDesignType, BindSegment)
            Next
        End Sub
        ...
        Private Function pToClipart(Category As cIItem.cItemCategoryEnum) As cItem
            ...
            Select Case Category
                Case cIItem.cItemCategoryEnum.Rock
                    Dim oItemClipart As cItemClipart = DirectCast(MyBase.Design.Layers(LayerTypeEnum.RocksAndConcretion), cLayerRocks).CreateRockFromClipart(Cave, Branch, oSVG.InnerXml, cItemClipart.cItemClipartDataFormatEnum.SVGData)
                ...
                Case cIItem.cItemCategoryEnum.Concretion
                    Dim oItemClipart As cItemClipart = DirectCast(MyBase.Design.Layers(LayerTypeEnum.RocksAndConcretion), cLayerRocks).CreateConcretionFromClipart(Cave, Branch, oSVG.InnerXml, cItemClipart.cItemClipartDataFormatEnum.SVGData)
                    Call oItemClipart.FixBound(True)
                    Dim oBounds As RectangleF = GetBounds()
                    Dim oCurrentCenter As PointF = modPaint.GetCenterPoint(oBounds)
                    oCurrentCenter.X -= oBounds.Width / 2
                ...
                Case cIItem.cItemCategoryEnum.Sign
                    Dim oItemSign As cItemSign = DirectCast(MyBase.Design.Layers(LayerTypeEnum.Signs), cLayerSigns).CreateSign(Cave, Branch, oSVG.InnerXml, cItemSign.cItemSignDataFormatEnum.SVGData)
