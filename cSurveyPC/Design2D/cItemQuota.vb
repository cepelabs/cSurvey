...
Namespace cSurvey.Design.Items
    Public Class cItemQuota
        ...
        Private iTextSize As TextSizeEnum
        ...
        Friend Overrides Sub Render(ByVal Graphics As System.Drawing.Graphics, ByVal PaintOptions As cOptions, ByVal Options As cItem.PaintOptionsEnum, ByVal Selected As SelectionModeEnum)
            ...
            With oCache
                If .Invalidated Then
                    ...
                    Dim oPen As Pen = New Pen(Color.FromArgb((1 - MyBase.Transparency) * 255, Color.Black))
                    oPen.Width = -1
                    Dim oWireframePen As Pen = oPen.Clone
                    oWireframePen.DashStyle = DashStyle.Dot

                    Dim oBoundPath As GraphicsPath = New GraphicsPath
                    ...
                    Select Case iQuotaType
                        Case cIItemQuota.QuotaTypeEnum.Vertical
                            Dim oArrowPen As Pen = oPen.Clone
                            Select Case iQuotaCapDecoration
                                ...
                                Case cIItemQuota.QuotaCapDecorationEnum.Circle
                                    oArrowPen.EndCap = LineCap.Round
                            End Select

                            Dim oQuotaPath As GraphicsPath = New GraphicsPath
                            Dim sQuotaGap As Single = oBounds.Width / 20
                            Select Case iQuotaAlign
                                Case cIItemQuota.QuotaAlignEnum.Left
                                    ...
                                    Call oQuotaPath.Dispose()

                                    oQuotaPath = New GraphicsPath
                                    Call oQuotaPath.StartFigure()
                        ...
                        Case cIItemQuota.QuotaTypeEnum.Horizontal
                            ...
                            Select Case iQuotaCapDecoration
                                Case cIItemQuota.QuotaCapDecorationEnum.Arrow
                                    ...
                                    oArrowPen.CustomEndCap = New AdjustableArrowCap(5, 5, True)
                                Case cIItemQuota.QuotaCapDecorationEnum.Circle
                                    oArrowPen.EndCap = LineCap.Triangle
                            End Select

                            Dim oQuotaPath As GraphicsPath = New GraphicsPath
                            Dim sQuotaGap As Single = oBounds.Height / 20
                            Select Case iQuotaAlign
                                Case cIItemQuota.QuotaAlignEnum.Left
                                    Call oQuotaPath.StartFigure()
                        ...
                        Case cIItemQuota.QuotaTypeEnum.Oblique
                            'Dim oQuotaPath As GraphicsPath = New GraphicsPath
                            ''Call oQuotaPath.StartFigure()
                            ''Call oQuotaPath.AddLine(New PointF(oBounds.Left, oBounds.Top), New PointF(oBounds.Left, oBounds.Bottom))
                            ''Call oQuotaPath.StartFigure()
                            ''Call oQuotaPath.AddLine(New PointF(oBounds.Right, oBounds.Top), New PointF(oBounds.Right, oBounds.Bottom))
                            ''Call oCache.Add(oQuotaPath, open, owireframepen, Nothing)
                            ''oQuotaPath = New GraphicsPath
                            'Dim sQuotaGap As Single = oBounds.Height / 20
                            'Select Case iQuotaAlign
                            '    Case cIItemQuota.QuotaAlignEnum.Left
                            ...
                            '        oCenterPoint = New PointF(oBounds.Left + sHalfWidth, oBounds.Top + sHalfHeight)
                            'End Select
                            'Call oCache.Add(oQuotaPath, oArrowPen, oArrowPen, Nothing)
                        Case cIItemQuota.QuotaTypeEnum.Drop
                            oCenterPoint = New PointF(oBounds.Left + sHalfWidth, oBounds.Top + sHalfHeight)
                        Case cIItemQuota.QuotaTypeEnum.Altitude
                            oCenterPoint = New PointF(oBounds.Left + sHalfWidth, oBounds.Top + sHalfHeight)
                            ...
                            Dim oQuotaPath As GraphicsPath
                            oQuotaPath = New GraphicsPath
                            Call oQuotaPath.StartFigure()
                        ...
                        Case cIItemQuota.QuotaTypeEnum.VerticalScale
                            ...
                            'Try
                            '    If MyBase.Design.Type = cIDesign.cDesignTypeEnum.Plan Then
                            '        sOffset = oSurvey.Calculate.TrigPoints(sQuotaRelativeTrigPoint).Point.To2DPoint(Calculate.cTrigPointPoint.ProjectionEnum.FromTop).Y - oBounds.Top
                            '    Else
                            '        sOffset = oSurvey.Calculate.TrigPoints(sQuotaRelativeTrigPoint).Point.To2DPoint(Calculate.cTrigPointPoint.ProjectionEnum.Perpendicular).Y - oBounds.Top
                            '    End If
                            'Catch
                            '    sOffset = 0
                            'End Try
                            ...
                            'Dim sLastTick As Single
                            'Dim iTickIndex As Integer = 0
                            'Dim iTickValue As Integer
                            'Dim sFirstTick As Single = oBounds.Top + (sOffset - CInt(sOffset))
                            'For sMeter As Single = sFirstTick To oBounds.Bottom Step 1
                            '    iTickValue = -1 * iTickIndex + CInt(sOffset)
                            '    If iTickValue Mod iQuotaTickFrequency = 0 Then
                            '        Call oQuotaPath.StartFigure()
                            '        Call oQuotaPath.AddLine(New PointF(sLeft, sMeter), New PointF(sRight, sMeter))
                            '    End If
                            '    If iTickValue Mod iQuotaTickLabelFrequency = 0 Then
                            '        Dim oQuotaTickTextPath As GraphicsPath = New GraphicsPath
                            '        Call oFont.AddToPath(PaintOptions, oQuotaTickTextPath, iTickValue, New PointF(0, 0), oSF)
                            ...
                            '    Case cIItemQuota.QuotaAlignEnum.Left
                            '        Call oQuotaPath.StartFigure()
                            '        Call oQuotaPath.AddLine(New PointF(sLeft, sFirstTick), New PointF(sLeft, sLastTick))
                            '    Case cIItemQuota.QuotaAlignEnum.Right
                            '        Call oQuotaPath.StartFigure()
                            '        Call oQuotaPath.AddLine(New PointF(sRight, sFirstTick), New PointF(sRight, sLastTick))
                            '    Case cIItemQuota.QuotaAlignEnum.Center
                            '        Call oQuotaPath.StartFigure()
                            '        Call oQuotaPath.AddLine(New PointF(sLeft + sQuotaTickSize / 2, sFirstTick), New PointF(sLeft + sQuotaTickSize / 2, sLastTick))
                            'End Select
                            'Call oCache.Add(cDrawCacheItem.cDrawCacheItemType.Border, oQuotaPath, oPen, oWireframePen, Nothing)
                            'Call oQuotaPath.Dispose()
                        Case cIItemQuota.QuotaTypeEnum.HorizontalScale
                            ...
                            'Try
                            '    If MyBase.Design.Type = cIDesign.cDesignTypeEnum.Plan Then
                            '        sOffset = oSurvey.Calculate.TrigPoints(sQuotaRelativeTrigPoint).Point.To2DPoint(Calculate.cTrigPointPoint.ProjectionEnum.FromTop).X + oBounds.Left
                            '    Else
                            '        sOffset = oSurvey.Calculate.TrigPoints(sQuotaRelativeTrigPoint).Point.To2DPoint(Calculate.cTrigPointPoint.ProjectionEnum.Perpendicular).X + oBounds.Left
                            '    End If
                            'Catch
                            '    sOffset = 0
                            'End Try
                            ...
                            'Dim iTickIndex As Integer = 0
                            'Dim iTickValue As Integer
                            'Dim sFirstTick As Single = oBounds.Left + (sOffset - CInt(sOffset))
                            'For sMeter As Single = sFirstTick To oBounds.Right Step 1
                            '    iTickValue = iTickIndex + CInt(sOffset)
                            '    If iTickValue Mod iQuotaTickFrequency = 0 Then
                            '        Call oQuotaPath.StartFigure()
                            '        Call oQuotaPath.AddLine(New PointF(sMeter, [sTop]), New PointF(sMeter, [sBottom]))
                            '    End If
                            '    If iTickValue Mod iQuotaTickLabelFrequency = 0 Then
                            '        Dim oQuotaTickTextPath As GraphicsPath = New GraphicsPath
                            '        Call oFont.AddToPath(PaintOptions, oQuotaTickTextPath, iTickValue, New PointF(0, 0), oSF)
                            ...
                            '    Case cIItemQuota.QuotaAlignEnum.Left
                            '        Call oQuotaPath.StartFigure()
                            '        Call oQuotaPath.AddLine(New PointF(sFirstTick, [sTop]), New PointF(sLastTick, [sTop]))
                            '    Case cIItemQuota.QuotaAlignEnum.Right
                            '        Call oQuotaPath.StartFigure()
                            '        Call oQuotaPath.AddLine(New PointF(sFirstTick, sBottom), New PointF(sLastTick, sBottom))
                            '    Case cIItemQuota.QuotaAlignEnum.Center
                            '        Call oQuotaPath.StartFigure()
                            '        Call oQuotaPath.AddLine(New PointF(sFirstTick, [sTop] + sQuotaTickSize / 2), New PointF(sLastTick, [sTop] + sQuotaTickSize / 2))
                            'End Select
                            'Call oCache.Add(cDrawCacheItem.cDrawCacheItemType.Border, oQuotaPath, oPen, oWireframePen, Nothing)
                            'Call oQuotaPath.Dispose()
                    End Select
        ...
        Friend Overrides Function SaveTo(ByVal File As Storage.cFile, ByVal Document As XmlDocument, ByVal Parent As XmlElement, Options As cSurvey.SaveOptionsEnum) As XmlElement
            ...
            If iTextSize <> TextSizeEnum.Default Then Call oItem.SetAttribute("textsize", iTextSize)
