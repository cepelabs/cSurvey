...
Namespace XSystem.Linq.Dynamic
    ...
    Friend Class Signature : Implements IEquatable(Of Signature)
        ...
        Public Overloads Function Equals(ByVal other As Signature) As Boolean Implements IEquatable(Of XSystem.Linq.Dynamic.Signature).Equals
        ...
        Function ParseComparison() As Expression
            ...
            Do While tokenVal.id = TokenId.Equal OrElse tokenVal.id = TokenId.DoubleEqual OrElse _
                ...
                ElseIf IsEnumType(left.Type) OrElse IsEnumType(right.Type) Then
                    If left.Type.IsEnum And Not right.Type.IsEnum Then
                        left = PromoteExpression(left, right.Type, True)
                    End If
                    If right.Type.IsEnum And Not left.Type.IsEnum Then
                        right = PromoteExpression(right, left.Type, True)
                    End If
                    If Not left.Type.Equals(right.Type) Then
...
amphersand:
                ...
                Else
                    ...
                    If literals.TryGetValue(ce, text) Then
                        ...
                        If value IsNot Nothing Then
                            If type.IsEnum Then
                                Return Expression.Constant(value, GetType(Int32))
                            Else
        ...
        Shared Function IsCompatibleWith(ByVal source As Type, ByVal target As Type) As Boolean
            ...
            'ATTENZIONE...rispetto alle specifiche MS gli enum vengono forzati a INT32...
            Dim sc As TypeCode = If(st.IsEnum, TypeCode.Int32, Type.GetTypeCode(st))
            Dim tc As TypeCode = If(tt.IsEnum, TypeCode.Int32, Type.GetTypeCode(tt))
