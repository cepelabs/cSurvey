...
Namespace cSurvey.Drawings
    Public Class cClipperRegion
        ...
        Public Sub New(Path As GraphicsPath, Optional Scale As Single = 1, Optional Accurancy As Single = 1)
            ...
            Dim oBasePath As GraphicsPath = Path.Clone
            Call oBasePath.Flatten(oBaseMatrix, sAccurancy)

            oBaseIntPaths = pGraphicsPathToIntPaths(oBasePath)
            Call oBasePath.Dispose()
        End Sub

        Private Function pGraphicsPathToIntPaths(Path As GraphicsPath) As List(Of List(Of IntPoint))
        ...
        Public Function Contains(Graphics As Graphics, Path As GraphicsPath) As Boolean Implements cIRegion.Contains
            ...
            Dim oIntPaths As List(Of List(Of IntPoint)) = pGraphicsPathToIntPaths(oPath)
            Call oPath.Dispose()
            Call oClipper.Clear()
