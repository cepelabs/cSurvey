...
friend Class frmMasterSlaveManager
    ...
    Private Sub cmdSetAsSlave_Click(sender As Object, e As EventArgs) Handles cmdSetAsSlave.Click
        ...
        '                Call oSlaveUser.CavesAndBranches.Append(oCaveInfo.Name, "", dDate, "ADMIN", "USER1")
        '                Call pAppendBranches(oCaveInfo.Branches, oCaveInfo.Locked, dDate, "ADMIN", "USER1", oSlaveUser.CavesAndBranches)
        '            End If
        '        Next
    ...
    Private Sub pAppendBranches(Branches As cCaveInfoBranches, ParentLocked As Boolean, [Date] As Date, AssignedBy As String, AssignedTo As String, CavesAndBranches As Master.cCaveAndBrancheLockInfos)
        For Each oBranch As cCaveInfoBranch In Branches
            If oBranch.Locked OrElse ParentLocked Then
                Call CavesAndBranches.Append(oBranch.Cave, oBranch.Path, [Date], AssignedBy, AssignedTo)
                Call pAppendBranches(oBranch.Branches, oBranch.Locked Or ParentLocked, [Date], AssignedBy, AssignedTo, CavesAndBranches)
            End If
        Next
    End Sub
